...
// Registering the polygon:// URL handler
searchbarPlugins.registerURLHandler(function (url) {
    if (url.startsWith('polygon://')) {
        // Handle opening Polygon addresses here
        var address = url.substring(10); // Remove "polygon://" prefix
        // Do something with the Polygon address, e.g., open it in a wallet app
        console.log("Opening Polygon address:", address);
        return true; // Return true if the URL is handled
    }
    return false; // Return false if the URL is not handled
});
